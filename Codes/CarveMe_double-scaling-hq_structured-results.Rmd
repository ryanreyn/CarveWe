...
#Trying something temporary (remove below lines before finalizing) where we also normalize the row sums to be all the same
...
initSOM(dimension=c(gridrows,gridcols),nb.save=10,scaling="none")
set.seed(123)
carve.som<-trainSOM(x.data=scaled_matrix,nb.save=10,scaling="none")
...
for (i in 1:length(sizes)){
  ...
  num_clusters<-curr.sc$cluster
  ids<-curr.sc$som$clustering
  tmp_sample_clusters<-num_clusters[ids]
...
cluster_plot <- ggplot() +
  ...
  geom_errorbar(data = metab_df, aes(x = cluster,ymin = means-sds, ymax = means+sds),
                width = 0.2) +
  facet_wrap(vars(`Metabolite Class`), ncol = 3, scales = "free_y") +
...
matti_dependencies<-read.csv(file="Data/model_growth_rate_info_by_met_category.csv",header=TRUE,row.names=1)%>%t()%>%as.data.frame()

#Currently this is in a weird format and needs to be averaged at each element
...
req_df<-sub_df
for (col in 2:dim(sub_df)[2]){
  req_df[,col]<-2*(1-(as.numeric(sub_df[,col])/as.numeric(sub_df[,1])))
}

req_df<-req_df%>%select(-c(growth_rates_own))%>%mutate(cluster=matti_dependencies$som_cluster,genome=rownames(req_df))%>%select(genome,cluster,everything())
rownames(req_df)<-c()
...
grow_df<-req_df%>%select(-c(cluster,genome,preference))%>%t()%>%as.data.frame()%>%mutate(metabolite_class=rownames(.),.before=everything())
rownames(grow_df)<-NULL; colnames(grow_df)[-1]<-req_df$genome

grow_df <- grow_df %>% pivot_longer(-metabolite_class, names_to = "genome",values_to = "growth_req") %>%
  filter(metabolite_class != "Inorganic")

grow_df$cluster <- req_df$cluster[match(grow_df$genome,req_df$genome)]
grow_df$preference<-req_df$preference[match(grow_df$genome,req_df$genome)]

sum_grow <- grow_df %>% group_by(preference,metabolite_class) %>%
  summarise(avg_grow_req = mean(growth_req, na.rm = TRUE)) 
...
#sum_grow$cluster <- factor(sum_grow$cluster)
sum_grow$preference<-factor(sum_grow$preference)
...
matti_growth_dep_plot <- ggplot(sum_grow, aes(x = preference, y = metabolite_class, size = scaled_grow, fill = avg_grow_req)) + 
  geom_point(pch = 21) + scale_size_continuous(guide = "none", range = c(2,10)) +
  scale_fill_viridis_c() +
  theme(panel.background = element_blank(),
        panel.border = element_rect(fill = NA, color = "black"),
...
### 3.4.4   Growth Dependency/Requirement
