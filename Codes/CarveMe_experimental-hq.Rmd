...
out_tree4 <- gheatmap2(t_plot4, som_df, offset= 18, width=0.05, colnames = FALSE) +
  scale_fill_manual(values = c("#cb5336",
                               "#49ae8a",
                               "#b45ac2",
                               "#7ca343",
                               "#6e7fcc",
                               ...
                               "#c94e6b"), 
                    na.translate = FALSE) + 
  scale_color_manual(values = c("#cb5336",
                                "#49ae8a",
                                "#b45ac2",
                                "#7ca343",
                                "#6e7fcc",
...
growth_df<-mutate(growth_df,matchrow=match(cluster_df$value,growth_dat$X,nomatch=0))
...
ggplot(growth_df,aes(x=as.factor(clusters),y=dCUB))+
  geom_violin(width=1)+
...
grow_df <- read.csv("Data/met_depend_factor_by_genome_all_microbiomics.csv")
...
grow_df$cluster <- clust_df$clusters[match(grow_df$genome,clust_df$value)]
...
sum_grow$metabolite_class <- factor(sum_grow$metabolite_class, levels = grow_order$metabolite_class[c(4,1:3,5:12)])
...
print("Figure: Potential growth limitation by each metabolite class colored by SOM cluster.")
#agg_png("growth_dependancy_plot.png", width = 8, height = 6, units = "in", res = 400)
plot(out_grow)
#dev.off()
...
out_plot2 <- ggplot(sum_grow, aes(x = cluster, y = metabolite_class, size = scaled_grow, fill = avg_grow_req)) + 
...
print("Figure: Alternative visualization of potential growth limitation (avg growth req) by metabolite class per cluster. Bubble colors reflect information shown in previous figure, while bubble sizes describe the relative importance of the limitation by each metabolite class relatively between clusters (i.e., larger bubble implies more important for that cluster relative to other clusters).")
#agg_png("metabolites_growth_bubble.png", width = 8, height = 6, units = "in", res = 400)
plot(out_plot2)
#dev.off()


# compare plot
mean_df <- mean_df[,c(2,5:16)] %>% pivot_longer(-cluster, names_to = "metabolite_class", values_to = "flux")
mean_df$cluster <- factor(mean_df$cluster)
comb_df <- full_join(sum_grow, mean_df)
...
ggplot(comb_df, aes(x = flux, y = avg_grow_req, color = cluster)) +
...
metab_df$`Metabolite Class` <- factor(metab_df$`Metabolite Class`,
                                     levels = metab_order$`Metabolite Class`[c(1:5,7:13,6)])

out_plot <- ggplot() +
  geom_hline(yintercept = 0, lty = 2) +
  geom_bar(data = metab_df, aes(y = means, x = cluster, fill = cluster),
           stat = "identity") +
...
# agg_png("metabolites_cluster_fig_y.png", width = 10, height = 9, units = "in", res = 400)
plot(out_plot)
# dev.off()
...
new_df$`Metabolite Class` <- factor(new_df$`Metabolite Class`,
                                     levels = metab_order$`Metabolite Class`[c(6,13:7,5:1)])

out_plot2 <- ggplot() +
  geom_vline(xintercept = factor(1:num_clusters), lty = 2, linewidth = 0.25) +
  geom_point(data = new_df, aes(x = cluster, y = `Metabolite Class`, fill = means, size = scaled_flux), pch=21) +
  scale_size_continuous(range = c(0.01,10), limits = c(0,1), guide = "none") +
  labs(x = "Cluster", fill = "Scaled Flux", size = "Relative Flux") +
  scale_fill_gradientn(colours = c("blue","white","red"), 
                       values = scales::rescale(c(-2.81,0,4)),
                       guide = "colorbar", limits=c(-2.81,4), breaks = c(-2,0,2,4)) +
  theme(panel.background = element_blank(),
        panel.border = element_rect(fill = NA, color = "black"),
        axis.title.y = element_blank(),
        axis.text = element_text(size = 12),
...
# agg_png("metabolites_cluster_bubble.png", width = 8, height = 6, units = "in", res = 400)
plot(out_plot2)
# dev.off()
...
plot_df$row_names <- (c(1,2,1,2,3,1,2,3)-1)*2+0.55
plot_df$column_names <- (c(1,1,2,2,2,3,3,3)-1)*2+0.25
...
split_phylo_hq<-split_phylo_hq%>%filter(Genome.prokka%in%genome_IDs)

phylo_piv <- split_phylo_hq[,2:13] %>% pivot_longer(-group, names_to = "K", values_to = "cluster") %>% 
...
ggplot(bray_df, aes(x = as.factor(K), y = bray)) + geom_violin(width = 1) +
...
## We also tried to benchmark our results using genomes cultivated by the Cordero Lab and analyzed by Matti Gralka. Matti characterized the sugar-acid specialty of these genomes and verified their growth (both capability and rate) on each. Mapping these onto our SOMs potentially could give insights into the accuracy of our clustering.
...
remove_metab<-which(colnames(scaled_matti_fluxes) %in% c("Manganese","Zinc","Co2+","Calcium","Chloride","Potassium","Copper","Magnesium"))
scaled_matti_fluxes<-scaled_matti_fluxes[,-remove_metab];
...
for (i in matti_genomes){
  ...
  averaged_fluxes<-rbind(averaged_fluxes,colMeans(scaled_fluxes[curr_rows,]))
}
...
all_deps<-all_deps%>%mutate(cluster=c(1:num_clusters),`Carb/Acid Ratio`=scale(`Carbohydrates/Derivatives`/`Carboxylic Acid`))%>%select(cluster,everything())
