...
mean_df$row_names <- rep(c(1:(num_clusters/2)),times=2)
mean_df$column_names <- rep(c(1:2),each=num_clusters/2)
...
plot_df$row_names <- rep(c(1:(num_clusters/2)),times=2)
plot_df$column_names <- rep(c(1:2),each=num_clusters/2)
...
growth.tukey <- TukeyHSD(x=ANOVA,'growth_df$clusters',conf.level=0.95)
Tukey.levels <- growth.tukey[["growth_df$clusters"]][,4]
growth.Tukey.labels <- data.frame(multcompLetters(Tukey.levels)['Letters'])
growth.Tukey.labels$treatment <- as.numeric(rownames(growth.Tukey.labels))
...
#Raw plot of the growth sensitivity on [0,1] scale for all compound classes
...
all_growth_info <- cbind(sugar_growth_info,acid_growth_info)%>%as.data.frame()
matti.metab.ids <- read.csv("Data/matti_lab_metabolite_ids.csv")
...
all_growth_info <- all_growth_info%>%mutate(Genome=raw_growth_file$X[-1])%>%select(Genome,everything())
piv.all_growth <- all_growth_info%>%pivot_longer(-c(Genome),names_to="Metabolites",values_to="Growth")%>%filter(Metabolites %in% metab_names, Metabolites != "oxalacetate",Genome %in% hq_genomes)
...
binary_df<-all_growth_info%>%pivot_longer(-c(Genome),names_to="Metabolites",values_to="Growth")%>%filter(Metabolites %in% metab_names, Metabolites != "oxalacetate")%>%mutate(`Growth/No Growth`=case_when(Growth > 0 ~ 1, .default=0))%>%select(-Growth)%>%pivot_wider(.,names_from=Metabolites,values_from=`Growth/No Growth`)
...
boot_plot<-ggplot(data = boot_df, aes(x = Agreement))+
  ...
  geom_vline(aes(xintercept = 0.736, color = "Model Agreement"), lty = 2, linewidth = 2)+
  scale_color_manual(name = "Statistics", values = c(`Bootstrap Mean` = "blue", `Model Agreement` = "orange"))+
  scale_x_break(c(0.63,0.725), ticklabels = c(seq(0.59,0.63,0.01),0.73,0.74))+
  labs(x = "Percent Agreement", y = "Density of Counts", title = "Agreement between Gralka's experimental data and randomized data")+
  expand_limits(x=c(0.59,0.74))+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5, size = 20), axis.title = element_text(size=18), axis.text = element_text(size=18))

matti.growth.heatmap <- ggplot()+
  geom_tile(data=piv.all_growth,aes(x=Metabolites,y=Genome,fill=norm_grow))+
  labs(fill="Growth")+
...
matti.binary.heatmap <- ggplot()+
  geom_tile(data=piv.all_growth,aes(x=Metabolites,y=Genome,fill=`Growth/No Growth`))+
  scale_fill_manual(labels=c("Growth","No Growth"),values=c("orange","gray90"))+
  labs(fill="Growth/No Growth")+
  theme_bw()+
  theme(axis.text.x=element_text(angle=270,hjust=0))+
  custom_theme

vitamin.rescue.growth <- read_csv(file="Data/matti_vit_gr_compare.csv")%>%rename(.,`Rescue Growth`=Growth_rates_max,`Growth/No Growth`=Growth)%>%filter(Genome %in% hq_genomes)

single.rescue.growth <- read_csv(file="Data/matti_high_gr_compare.csv")%>%rename(.,`Single Rescue Growth`=Growth_rates_max,`Single Growth/No Growth`=Growth)%>%filter(Genome %in% hq_genomes)

double.rescue.growth <- read_csv(file="Data/matti_dbl_gr_compare.csv")%>%rename(.,`Double Rescue Growth`=Growth_rates_max,`Double Growth/No Growth`=Growth)%>%filter(Genome %in% hq_genomes)

matti.model.growth <- left_join(single.rescue.growth,double.rescue.growth,by=join_by(Genome==Genome,`Tested Compound`==`Tested Compound`))%>%
  mutate(`Double Rescue Growth`=case_when(
    is.na(`Double Rescue Growth`) == TRUE ~ 0,
    .default=`Double Rescue Growth`
  ),`Double Growth/No Growth`=case_when(
    is.na(`Double Growth/No Growth`) == TRUE ~ 0,
    .default=`Double Growth/No Growth`
  ))%>%
  mutate(`Rescue Growth`=pmax(`Single Rescue Growth`,`Double Rescue Growth`), `Growth/No Growth`=pmax(`Single Growth/No Growth`,`Double Growth/No Growth`))%>%
  select(Genome,`Tested Compound`,`Rescue Growth`,`Growth/No Growth`)

matti.model.growth <- matti.model.growth%>%
  left_join(.,select(matti.metab.ids,c(X,Exchange_rxn)),by=join_by(`Tested Compound`==Exchange_rxn))%>%
  rename(.,Metabolites=X)%>%
  select(Genome,`Tested Compound`,Metabolites,everything())

matti.model.growth <- matti.model.growth%>%
  group_by(Genome)%>%
  mutate(norm_grow=case_when(
    max(`Rescue Growth`)>0 ~ `Rescue Growth`/max(`Rescue Growth`),
    .default=0))

model.binary.heatmap <- ggplot()+
  geom_tile(data=matti.model.growth,aes(x=as.factor(Metabolites),y=Genome,fill=as.factor(`Growth/No Growth`)))+
  scale_fill_manual(labels=c("No Growth","Growth"),values=c("gray90","orange"),na.value="gray90")+
  labs(x="Metabolites", fill="Growth/No Growth")+
  theme_bw()+
  theme(axis.text.x=element_text(angle=270,hjust=0,vjust=0.5))+
  custom_theme

model.growth.heatmap <- ggplot()+
  geom_tile(data=matti.model.growth,aes(x=Metabolites,y=Genome,fill=norm_grow))+
  labs(fill="Growth")+
  scale_fill_gradient(low="blue",high="red")+
  theme_bw()+
  theme(axis.text.x=element_text(angle=270,hjust=0,vjust=0.5))+
...
agree_df <- read.csv("Data/20241202_matti_agreement_final.csv")
...
model=lm(match.agree.df$norm_matti ~ match.agree.df$norm_bin)
ANOVA=aov(model)

TUKEY <- TukeyHSD(x=ANOVA,'match.agree.df$norm_bin',conf.level=0.95)
Tukey.levels <- TUKEY[["match.agree.df$norm_bin"]][,4]
growth.Tukey.labels <- data.frame(multcompLetters(Tukey.levels)['Letters'])
growth.Tukey.labels$treatment <- rownames(growth.Tukey.labels)
numeric.labels <- match(growth.Tukey.labels[,1],unique(growth.Tukey.labels[,1]))

match.agree.df <- match.agree.df%>%mutate(group=growth.Tukey.labels$Letters[match(norm_bin,growth.Tukey.labels$treatment)])
match.agree.df$group <- factor(match.agree.df$group,levels=c("a","b"))
...
# binned.raw.compare.plot <- ggplot()+
#   geom_boxplot(data=match.agree.df,aes(x=raw_bin, y=growth_rate))+
#   labs(x="Model Predicted Growth",y="Matti Predicted Growth",title="Binned Raw Growth Comparison")+
#   custom_theme

binned.norm.compare.plot <- ggplot()+
  geom_boxplot(data=match.agree.df,aes(x=norm_bin, y=norm_matti,color=group),size=1.5)+
...
# agg_png(filename="Figures/final-figs/binned-growth-comparison-plot.png",res=400,units="mm",height=150,width=300)

#I also want to compute the precision, accuracy, and recall of our method compared to matti's experimental data
sub.agree_df <- agree_df%>%select(c(Genome,Compound,Metabolite,match_key,agreement_status))
sub.agree_df <- left_join(sub.agree_df,piv.all_growth,by=join_by(Genome == Genome, match_key == Metabolites))

only_agreements <- sub.agree_df%>%filter(agreement_status %in% c("Perfect Agreement","Single Rescue","Double Rescue"))
# only_agreements <- left_join(only_agreements,piv.binary_df,by=join_by(Genome == Genome, Compound == Metabolites))

comparison.accuracy <- sub.agree_df%>%summarise(accuracy=sum(agreement_status%in%c("Perfect Agreement","Single Rescue","Double Rescue"))/n())

true_positives <- only_agreements%>%filter(`Growth/No Growth`==1)%>%summarise(count=n())%>%as.matrix()
false_positives <- sub.agree_df%>%filter(agreement_status=="Model Growth, Exp. No Growth")%>%summarise(count=n())%>%as.matrix()
false_negatives <- sub.agree_df%>%filter(agreement_status%in%c("No Growth, Pathway Present","No Growth, No Pathway"))%>%summarise(count=n())%>%as.matrix()

comparison.precision <- true_positives/(true_positives+false_positives)

comparison.recall <- true_positives/(true_positives+false_negatives)

#Rearrange the original agreement df by class and classification
agree_df <- agree_df%>% arrange(Higher.Level.Classification)
class <- unique(agree_df$Higher.Level.Classification)

# reorder classes
class <- class[c(2,1,3,4,5,6)]
...
matti.agree.heatmap <- ggplot() +
  geom_tile(data = agree_df, aes(x = Metabolite, y = Genome, fill = agreement_status), color = "white") +
...
tukey_df <- growth.tukey[["growth_df$clusters"]]%>%as.data.frame()
...
knitr::include_graphics("Figures/final-figs/consensus90-sensitivity-bubble-plot.png")
