...
knitr::opts_knit$set(root.dir="/Users/Ryan/Documents/Levine_Lab/R_Studio/CarveWe/reproduce_publication")
#Load in packages
library(SOMbrero)
library(gplots)
library(tidyverse)
...
#Call functions
source(file="Codes/CarveWe_functions.R")
...
#Test a version of SOMs using only one of the models instead of all 60
# all_dat <- all_dat%>%
#   filter(model==30)
...
#We want to explicitly exclude cyanobacteria from the analysis now
#We also did an analysis of sensitivity variance and removed the 100 most variable genomes, load in that dataset. This dataset also excludes cyanos
load("Publication_Data/low-var-genomes.RData")
load("Publication_Data/hq-cyano-genomes.RData")
adjusted_dat <- adjusted_dat%>%
  filter(!genome %in% cyano_genomes,
         genome %in% low_var_genomes)

piv.grow_df <- adjusted_dat%>%
  filter(metab_class=="Growth")%>%
...
colnames(piv.grow_df) <- c("genome","model","metabolite_class","growth_req")

all.grow_df <- piv.grow_df%>%
  pivot_wider(names_from=metabolite_class,values_from=growth_req)

#Convert dataframe to a matrix for SOM mapping
...
#Area to generate custom matrices if needed
# metabolite_matrix <- all.grow_df%>%
#   filter(genome %in% bacterial.hq.genomes & genome %in% low_var_genomes)%>%
#   select(-c(genome,model))%>%
#   as.matrix()
# 
# rownames(metabolite_matrix) <- all.grow_df%>%
#   filter(genome %in% bacterial.hq.genomes & genome %in% low_var_genomes)%>%
#   select(genome)%>%
#   as.matrix()

#Generate the SOM map, change pre.gen to 0 if you want a new map and keep at 1 if you want pre-loaded maps
pre.gen=0

if (pre.gen == 0){
  som.grid <- somgrid(xdim=gridsize,ydim=gridsize,topo="hexagonal",toroidal=TRUE)
  som.output <- generate.soms(metabolite_matrix, som.grid)
  
  som.model <- som.output[[1]]
  carve.som <- som.output[[2]]
  som.dist <- som.output[[3]]
...
#If the flag is checked, load the pre-saved cluster data
if (pre.gen == 0){
  cluster.output <- generate.clusters(som.model, num_clusters = num_clusters)
  
  som.cluster.k <- cluster.output[[1]]
  sample_clusters <- cluster.output[[2]]
  residuals_plot <- cluster.output[[3]]
...
```{r Publication SOM map plot, echo = FALSE}
nrow <- 20
ncol <- 20
...
for (i in 1:ncol) {
  for (j in 1:nrow) {
    ...
    if (exists("hex_df") == FALSE) {
      hex_df <- data.frame(x_cor = c(j_set,(j_set + r*(sqrt(3)/2)),(j_set + r*(sqrt(3)/2)),j_set, (j_set - r*(sqrt(3)/2)),(j_set - r*(sqrt(3)/2))),
                           y_cor = c(i_set+r, i_set + 0.5*r,i_set - 0.5*r,i_set-r,i_set - 0.5*r,i_set + 0.5*r),
                           i_set = i_set, j_set = j_set, i = i, j = j)
      hex_df$cluster <- som.cluster.k[(i-1)*ncol + j]
      hex_df$num <- (i-1)*ncol + j
      
   
    }else{
      temp_df <- data.frame(x_cor = c(j_set,(j_set + r*(sqrt(3)/2)),(j_set + r*(sqrt(3)/2)),j_set, (j_set - r*(sqrt(3)/2)),(j_set - r*(sqrt(3)/2))),
                            y_cor = c(i_set+r, i_set + 0.5*r,i_set - 0.5*r,i_set-r,i_set - 0.5*r,i_set + 0.5*r),
                            i_set = i_set, j_set = j_set, i = i, j = j)
      temp_df$cluster <- som.cluster.k[(i-1)*ncol + j]
...
hex_df$x_cor[which(hex_df$i/2 == floor(hex_df$i/2))] = hex_df$x_cor[which(hex_df$i/2 == floor(hex_df$i/2))] + 0.44

hex_df$alpha_val <- 1
hex_df$cluster <- factor(hex_df$cluster, levels = c("1","2","3","4","5","6","7","8"))
...
hex_line_i_low <- hex_sub_i_low %>% filter(y_cor > -0.7) %>% arrange(x_cor)
hex_line_i_low$y_cor <- hex_line_i_low$y_cor + 0.01

hex_sub_i_high <- hex_df %>% filter(i >= 18)
hex_sub_i_high$num <- paste(hex_sub_i_high$num, "reph")
hex_sub_i_high$y_cor <- hex_sub_i_high$y_cor + 15.5
hex_sub_i_high$alpha_val[which(hex_sub_i_high$i == 20)] <- 0.6
hex_sub_i_high$alpha_val[which(hex_sub_i_high$i == 19)] <- 0.4
hex_sub_i_high$alpha_val[which(hex_sub_i_high$i == 18)] <- 0.2

hex_line_i_high <- hex_sub_i_high %>% filter(y_cor < 15.5) %>% arrange(x_cor)
hex_line_i_high$y_cor <- hex_line_i_high$y_cor - 0.01
...
hex_line_j_left <- hex_sub_j_left %>% filter(x_cor < 18.5, y_cor < 15) %>% arrange(y_cor)
hex_line_j_left$y_cor <- hex_line_j_left$y_cor - 0.01

hex_sub_j_right <- hex_df %>% filter(j >= 18)
hex_sub_j_right$num <- paste(hex_sub_j_right$num, "repr")
hex_sub_j_right$x_cor <- hex_sub_j_right$x_cor - 17.5
hex_sub_j_right$alpha_val[which(hex_sub_j_right$j == 20)] <- 0.6
hex_sub_j_right$alpha_val[which(hex_sub_j_right$j == 19)] <- 0.4
hex_sub_j_right$alpha_val[which(hex_sub_j_right$j == 18)] <- 0.2

hex_line_j_right <- hex_sub_j_right %>% filter(x_cor > 0.2) %>% arrange(y_cor)
hex_line_j_right$y_cor <- hex_line_j_right$y_cor - 0.01

hex_new <- bind_rows(list(hex_df, hex_sub_i_high, hex_sub_i_low, hex_sub_j_left, hex_sub_j_right))


hex_line <- bind_rows(hex_line_i_high, hex_line_j_left, hex_line_i_low, hex_line_j_right)
hex_line <- hex_line %>% arrange(x_cor)

out_plot <- ggplot() +
  geom_polygon(data = hex_new, aes(x = x_cor, y = y_cor, group = num,
                                  fill = as.factor(cluster), alpha = alpha_val),color = "white") +
  geom_mark_hull(data = hex_df, aes(x = x_cor, y = y_cor), color = "black",
                 lty = 1, fill = NA, expand = 0, radius = 0, concavity = 0.6, lwd = 1.5) +
  theme_void() + labs(fill = "Cluster") +
  scale_fill_manual(values = cluster_colors, na.value = "grey70") + coord_fixed() +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 14),
        legend.title = element_text(size = 14),
        plot.margin = ggplot2::margin(10,10,10,10)) +
  guides(alpha = "none", fill = guide_legend(nrow = 1,theme = theme(legend.title.position = "top", legend.title = element_text(hjust = 0.5),
                                                           legend.position = "bottom", legend.direction = "horizontal")))

agg_png("figures/som_map.png", width = 8, height = 8, units = "in", res =400) 
plot(out_plot)
dev.off()
...
```{r SOM bootstrapping analysis, echo = FALSE}
run_analysis = 0
if (run_analysis == 1){
  #Code block for testing generating many random SOM maps for individual models and looking to see what clusters are conserved together
  #Build a save matrix of all genome-genome comparisons
  genome_IDs <- unique(rownames(metabolite_matrix))
  num_iter = 100
  grouping.info <- matrix(data = 0, nrow = length(genome_IDs), ncol = length(genome_IDs))
  cluster.info <- matrix(data=0, nrow=length(genome_IDs), ncol=num_iter)
  
  set.seed(123)
  for (i in 1:num_iter){
    rand_model = sample(c(0:59), 1)
    test.matrix <- all.grow_df%>%
      filter(model == rand_model)%>%
      select(-c(genome,model))%>%
      as.matrix()
    som.grid <- somgrid(xdim=gridsize,ydim=gridsize,topo="hexagonal",toroidal=TRUE)
    som.output <- generate.soms(test.matrix, som.grid)
    ...
    cluster.output <- generate.clusters(som.model)
    ...
    for (clust in 1:num_clusters){
      curr_indices <- which(sample_clusters == clust)
      curr_genomes <- genome_IDs[curr_indices]
      
      curr_combs <- combn(curr_indices, 2)%>%
        t()
      
      grouping.info[curr_combs] <- grouping.info[curr_combs] +1
    }
    
    cluster.info[,i] <- sample_clusters
  }
  #Compute ARI for all pairs of unique clusterings
  ari_mat <- matrix(data=NA, nrow=num_iter,ncol=num_iter)
  for (i in 1:num_iter){
    for (j in 1:num_iter){
      ari_mat[i,j] <- adjustedRandIndex(cluster.info[,i], cluster.info[,j])
    }
  }
  
  rand.info <- matrix(data=NA, nrow=length(genome_IDs), ncol=num_iter)
  for (i in 1:num_iter){
    rand_clusters <- sample(c(1:8),length(genome_IDs), replace = TRUE)
    rand.info[,i] <- rand_clusters
  }
  
  external_ari <- c()
  for (i in 1:num_iter){
    external_ari[i] <- adjustedRandIndex(rand.info[,i], cluster_df$clusters)
  }
  
  grouping.df <- as.data.frame(grouping.info)%>%
    mutate(genome1 = genome_IDs)%>%
    select(genome1, everything())
  colnames(grouping.df)[-1] <- genome_IDs
  
  piv.grouping <- grouping.df %>% pivot_longer(-genome1, names_to = "genome2", values_to = "weight")
  
  filter.piv <- piv.grouping%>%
    filter(weight == num_iter)
  
  graph.filter <- graph_from_data_frame(filter.piv, directed = FALSE)
  components.list <- components(graph.filter)
    
  #Pull out the weights for edge pairs in the graph and then save them
  found_weights <- lapply(combinations_to_find, function(edge_pair) {
      # Get the edge ID for the given vertex pair
      if (! edge_pair[1] %in% V(g)$name || ! edge_pair[2] %in% V(g)$name){
          return(NA) 
      } else {
          edge_id <- get.edge.ids(g, vp = edge_pair, error=FALSE)
          if (edge_id > 0) {
              return(E(g)[edge_pair[1] %--% edge_pair[2]]$weight)
          } else {
              return(NA) # Or handle non-existent edges as needed
          }
      }
  })
  names(found_weights) <- sapply(combinations_to_find, paste, collapse = "-")
}
...
for (i in 1:length(genome_IDs)){
  ...
  if (!is_empty(dim(dupe_check))==TRUE && dim(dupe_check)[1]==1){
...
split_phylo <- phylo%>%
  mutate(GTDB.Taxonomy=gsub("[a-z]__","",GTDB.Taxonomy))%>%
  separate(.,GTDB.Taxonomy,sep=";",into=c("domain","phylum","class","order","family","genus","species"))%>%
  filter(domain!="Archaea")

#Filter out cyanos
cyanobacteria.genomes <- split_phylo%>%
  filter(phylum == "Cyanobacteriota")%>%
  select(Genome.prokka)%>%
  as.matrix()

split_phylo <- split_phylo%>%
filter(phylum != "Cyanobacteriota")
...
taxa_df <- data.frame(taxonomy = split_phylo$group[match(tree$tip.label, split_phylo$Genome.prokka)])
...
tax_labs$labs <- split_phylo$group[match(tax_labs$id,split_phylo$Genome.prokka)]
...
out_tree2 <- gheatmap2(t_plot2, ref_df, offset= 4, width=0.1, colnames = FALSE) +
  ...
  scale_color_manual(values = c("red"), 
                     na.translate = FALSE, guide = "none") + labs(fill = "", color = "")
...
qual_df <- qual_df%>%
  filter(!rownames(qual_df) %in% cyanobacteria.genomes)
...
out_tree3 <- gheatmap2(t_plot3, qual_df, offset= 14, width=0.1, colnames = FALSE) +
  ...
  scale_color_gradient(low="gray99",high="black",na.value = "white", breaks = c(0,0.25,0.5,0.75,1), guide = "none") +
...
all.metab.df <- piv.grow_df

all.metab.df <- left_join(all.metab.df,cluster_df,by=join_by("genome"=="value"))

#Filter any possible NAs that could arise
all.metab.df <- all.metab.df%>%
  filter(!is.na(clusters))

#Plot to show the cumulative variance of each genome against consensus
var.df <- all.metab.df%>%
  group_by(genome, metabolite_class)%>%
  summarise(var = var(growth_req))%>%
  group_by(genome)%>%
  summarise(cum_var = sum(var))

var.df <- left_join(var.df, select(quality_data, c(X, mean_freq)), by = join_by("genome" == "X"))

var.plot <- ggplot()+
  geom_point(data = var.df, aes(x = cum_var, y = mean_freq), size = 2, alpha = 0.75)+
  geom_vline(xintercept = 0.1, linetype = 2)+
  theme_bw()+
  custom_theme+
  labs(x = "Cumulative variance across metabolite classes", y = "Consensus")

#Plot that shows the % of genomes with substantial sensitivity to each class in ranked order
high.sens.df <- all.metab.df%>%
  group_by(genome, metabolite_class)%>%
  summarise(mean = mean(growth_req))%>%
  group_by(metabolite_class)%>%
  summarise(perc = sum(mean >= 0.8)/n())

high.sens.df <- high.sens.df%>%
  mutate(metabolite_class = fct_reorder(metabolite_class, perc, .desc = TRUE))

 metab.high.sens.plot <- ggplot(high.sens.df,aes(x=metabolite_class,y=perc,fill=metabolite_class))+
  ...
  theme_bw()+
  custom_theme+
  theme(axis.text.x=element_blank(),axis.ticks.x=element_blank(),legend.position=c(0.699,0.75),legend.text=element_text(size=14),legend.title=element_text(hjust=0.5))+
...
sub_df <- sub_df%>%mutate(Group=case_when(
  clusters%in%c(1,8) ~ "slow - intermediate", 
  clusters == 5 ~"slow", 
  clusters == 6 ~ "fast", 
  clusters%in%c(2, 4, 7) ~ "intermediate", 
  clusters==3 ~ "intermediate - fast"))
...
flux_bubble_plot <- ggplot() +
  ...
  scale_color_manual(values=c("lightblue","#00BA38","yellowgreen","#619CFF","#F8766D"))+
  scale_y_discrete(limits=rev)+
  theme(panel.background = element_blank(),
        panel.border = element_rect(fill = NA, color = "black"),
        axis.title.y = element_blank(),
        axis.text = element_text(size = 20),
        axis.text.x = element_text(face="bold",size=20,color=c("lightblue","#00BA38","yellowgreen","#00BA38","#619CFF","#F8766D","#00BA38","lightblue")),
...
split_phylo <- phylo%>%
  mutate(GTDB.Taxonomy=gsub("[a-z]__","",GTDB.Taxonomy))%>%
  separate(.,GTDB.Taxonomy,sep=";",
           into=c("domain","phylum","class","order","family","genus",
                  "species"))%>%
  filter(domain != "Archaea", phylum != "Cyanobacteriota")
...
split_phylo_hq <- split_phylo%>%
  filter(Genome.prokka %in% genome_IDs)

groups <- split_phylo_hq%>%group_by(order)%>%summarise(count=n())%>%arrange(desc(count))

groups$type <- "Other"
groups$type[1:15] <- groups$order[1:15]
split_phylo_hq$group <- groups$type[match(split_phylo_hq$order,groups$order)]

split_phylo_hq <- split_phylo_hq%>%
  select(Genome.prokka, group)
...
split_phylo_hq <- split_phylo_hq%>%
  left_join(., cluster_df, by = join_by("Genome.prokka" == "value"))
phylo_mat <- matrix(data=0,nrow=num_clusters,ncol=16)
colnames(phylo_mat) <- unique(split_phylo_hq$group)
for (i in 1:num_clusters){
  curr_cluster <- split_phylo_hq%>%select(clusters,group)%>%filter(clusters==i)
  curr_phylo <- table(curr_cluster$group)
  phylo_cols <- match(names(curr_phylo),colnames(phylo_mat))
  phylo_mat[i,phylo_cols] <- curr_phylo
}
phylo_df <- as.data.frame(phylo_mat,row.names = FALSE)%>%mutate(.,clusters=c(1:num_clusters))%>%select(clusters,everything())
...
phylo_df$clusters <- c(1:num_clusters)

#Optional block of code for if you want to threshold

phylo_df <- phylo_df%>%select(.,c(nodes,clusters,row_names,column_names),everything())

pivot_df <- phylo_df%>% pivot_longer(-c(nodes, clusters, row_names, column_names),
...
pivot_df <- pivot_df%>% group_by(clusters)%>% mutate(perc = `Number of Genomes`/sum(`Number of Genomes`))
...
phylo_abund_plot <- ggplot() +
  geom_bar(data=pivot_df, aes(x = as.factor(clusters), y = perc, fill = Order), stat = "identity") + coord_cartesian(expand = FALSE) + labs(x = "Cluster", y = "Proportion")+
  scale_fill_manual(values = c("#b8617c",
                               "#63b750",
                               "#895bc9",
                               "#b2b53b",
  ...
  geom_point(data = legend_data, aes(x = 1, y = 1, color = label), size=0)+
  scale_color_manual(name = "Group", values = setNames(legend_data$color, legend_data$label))+
  theme(legend.position="bottom",axis.text.x=element_text(size=20,face="bold",color=c("lightblue","#00BA38","yellowgreen","#00BA38","#619CFF","#F8766D","#00BA38","lightblue")),legend.text=element_text(size=20),axis.title.x=element_text(size=20),axis.title.y=element_text(size=20),axis.text.y=element_text(size=20),legend.title=element_text(size=20, hjust=0.5))+
  guides(color=guide_legend(title.position="top", ncol=1, order=1, override.aes=list(size=5)),
         fill=guide_legend(title.position="top", order=2))

plot_df$row_names <- (rep(c(1:(num_clusters/2)),times=2)-1)*2+0.55
plot_df$column_names <- (rep(c(1:2),each=num_clusters/2)-1)*2+0.25
plot_df$clusters <- phylo_df$clusters
plot_df <- select(plot_df,nodes,clusters,row_names,column_names,everything())
...
phylo_grid_plot <- ggplot() +
  geom_col(data = pivot_df,
           aes(0,0, fill = Order))+
  geom_point(data = plot_df[,1:4], aes(x = column_names, y = row_names,color=as.factor(clusters)),
...
phylo_stacked_abund_plot <- ggplot(pivot_df, aes(y = Order, x = perc, fill = as.factor(clusters))) +
...
#Now plot
phylo_rel_diff_plot <- ggplot(pivot_df, aes(y = as.factor(clusters), x = diff, fill = as.factor(clusters))) +
...
num_models <- length(unique(all.grow_df$model))
points <- points%>%mutate(dCUB=rep(growth_df$dCUB,each=num_models))
...
growth.wilcoxon.list <- kruskal_test(growth_df, vars)
tukey.labels <- growth.wilcoxon.list[[3]]
numeric.labels <- growth.wilcoxon.list[[4]]
...
growth_df <- growth_df%>%
  mutate(plot_group = case_when(
    group == 3 ~ "c: intermediate - fast",
    group == 2 ~ "bc: intermediate",
    group == 1 ~ "ab: slow - intermediate",
    group == 4 ~ "a: slow",
    group == 5 ~ "d: fast"
  ))
growth_df$plot_group <- factor(growth_df$plot_group, 
                               levels = c("a: slow",
                                          "ab: slow - intermediate",
                                          "bc: intermediate",
                                          "c: intermediate - fast",
                                          "d: fast"))

growth_violin_plot <- ggplot(growth_df,aes(x=as.factor(clusters),y=dCUB,color=plot_group))+
  geom_violin(width=1)+
  stat_summary(fun="mean",geom="crossbar",color="black")+
  labs(x="Clusters")+
  scale_color_manual(name="Group",values=c("#619CFF","lightblue","#00BA38","yellowgreen","#F8766D"))+
  geom_point()+
  custom_theme+
  theme_bw()+
...
simple.model <- lm(growth_mean ~ flux_mean, avg_df)
r.squared = summary(simple.model)$adj.r.squared

flux_versus_growth_plot <- ggplot()+
  geom_point(data=avg_df, aes(x = flux_mean, y = growth_mean, color = clusters), size = 5)+
  geom_smooth(data = avg_df, aes(x = flux_mean, y = growth_mean), method = "lm", se = FALSE, color = "black", linetype = 2)+
  scale_color_manual(values = cluster_colors)+
  theme_bw()+
  labs(x = "Mean growth sensitivity", y = "Mean dCUB", color = "Clusters")+
  custom_theme+
  annotate("text", x = 0.05, y = -0.08, hjust = 1, vjust = 0, label = paste0("R² = ", round(r.squared, 2)), size = 5)
...
per.clust.phylo.df <- pivot_df%>%mutate(tukey_group=tukey.labels$Letters[match(clusters,tukey.labels$treatment)])
per.clust.levels <- per.clust.phylo.df%>%filter(clusters==1)%>%arrange(desc(perc))%>%ungroup()%>%select(Group)%>%as.matrix()%>%as.vector()
...
piv.all_growth <- all_growth_info%>%
  ...
  filter(Metabolites %in% metab_names, 
         Metabolites != "oxalacetate", 
         !Metabolites %in% polymer.compounds, 
         Genome %in% hq_genomes)
...
##### Temp script piece to test Reviewer 1's disagreement with bootstrap construction #####
#We want to randomize the same number of 1 values per genome in the bootstrapped artificial datasets
per_resource_ones <- rowSums(binary_mat)
...
for (i in 1:num_bootstraps){
  #Initialize the randomized data as all 0s
  randomized_mat <- matrix(data = 0, nrow = dim(binary_mat)[1], ncol = dim(binary_mat)[2])
  for (i in 1:dim(randomized_mat)[2]){
    #Identify the number of ones to use for each genomes, randomize that many for each row
    curr_ones <- per_resource_ones[i]
    rand_positions <- sample(c(1:dim(randomized_mat)[1]), curr_ones, replace = FALSE)
    
    randomized_mat[rand_positions,i] <- 1
  }
  #Compute the agreement between the real and randomized data
  agreement_mat <- binary_mat - randomized_mat
  perc_agreement <- sum(rowSums(agreement_mat == 0))/(nrow(binary_mat)*ncol(binary_mat))
...
#Now we want to make a pretty figure of the distribution and then highlight that we are doing much better than this distribution
resource_boot_plot<-ggplot(data = boot_df, aes(x = Agreement))+
  geom_histogram(aes(y = after_stat(density)), binwidth=0.001, fill = "grey75", color = "black")+
  stat_function(fun = dnorm, args = list(mean = mean(boot_df$Agreement), sd = sd(boot_df$Agreement)), linewidth = 2)+
  stat_function(fun = dnorm, args = list(mean = mean(boot_df$Agreement), sd = sd(boot_df$Agreement)), xlim = c(mean(boot_df$Agreement) + 2*sd(boot_df$Agreement), max(boot_df$Agreement)), geom = "area", alpha = 0.5, fill = "blue", linewidth = 2)+
  stat_function(fun = dnorm, args = list(mean = mean(boot_df$Agreement), sd = sd(boot_df$Agreement)), xlim = c(min(boot_df$Agreement), mean(boot_df$Agreement) - 2*sd(boot_df$Agreement)), geom = "area", alpha = 0.5, fill = "blue", linewidth = 2)+
  ...
  scale_color_manual(name = "Statistics", values = c(`Bootstrap Mean` = "blue", `Model Agreement` = "orange"))+
  scale_x_break(c(0.63,0.745), ticklabels = c(seq(0.59,0.63,0.01),0.75,0.76))+
  labs(x = "Proportion of Agreement", y = "Density of Counts", title = "Agreement between Gralka's experimental data and randomized data (column randomization)")+
  expand_limits(x=c(0.585,0.76))+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5, size = 20), axis.title = element_text(size=18), axis.text = element_text(size=18))
...
compare.raw.growth.plot <- ggplot()+
  ...
  labs(x="Model Predicted Growth",y="Gralka et al. Predicted Growth",title="Raw Growth Comparison")+
  custom_theme

compare.norm.growth.plot <- ggplot()+
  geom_point(data=match.agree.df,aes(x=norm_model,y=norm_matti),size=2,alpha=0.5)+
  labs(x="Model Predicted Growth",y="Gralka et al. Predicted Growth",title="Genome Normalized Growth Comparison")+
  custom_theme
...
binned.norm.compare.plot <- ggplot()+
  ...
  labs(x="Model Predicted Growth",y="Gralka et al. Predicted Growth",color="Statistical Group",title="Binned Normalized Growth Comparison")+
...
rpkm_vals <- read.csv("Publication_Data/revised-rpkm-output.csv")
...
som_rpkm <- rpkm_vals%>% 
  filter(!is.na(cluster),!is.na(rpkm))%>% 
...
#Attempt to standardize RPKM by dividing by mean total RPKM in each region and recomputing relative RPKM values
som_rpkm <- som_rpkm%>%
  left_join(., select(meta, c(sra_acc, db, depth, lat)), by = join_by(Sample == sra_acc, depth, lat))%>%
  distinct()%>%
  group_by(db, Region)%>%
  mutate(adjusted_rpkm = sum_rpkm/mean(sum_rpkm))%>%
  ungroup()%>%
  group_by(Sample)%>%
  mutate(rel_rpkm = adjusted_rpkm/sum(adjusted_rpkm, na.rm = TRUE))
...
#Testing a global reordering colored by Regime on top of plot to see if there's a strong pattern we can report
all_order <- c()
piv.rel.df <- sample.som_rpkm%>%
  select(cluster, Sample, rel_rpkm, SuperRegion)%>%
  pivot_wider(names_from=cluster, values_from=rel_rpkm)

stations <- piv.rel.df$Sample%>%as.vector()
piv.rel.mat <- piv.rel.df%>%
  ungroup()%>%
  select(-c(Sample,SuperRegion))%>%
  as.matrix()
all_dist <- vegdist(piv.rel.mat, method="bray")
all_clust <- hclust(all_dist,method="mcquitty")
all_order <- stations[all_clust$order]
...
test_plot <- ggplot(sample.som_rpkm,aes(x=Sample,y=rel_rpkm,fill=cluster))+
  ...
  new_scale_fill()+
  geom_tile(aes(x=Sample,y=1.005,fill=SuperRegion))+
  scale_fill_manual(values=c("red","orange","lightblue","blue","darkblue"))+
    theme_bw()+
...
for(i in c("Estuarine","Coastal","Oligotrophic Seas","Oligotrophic Open Ocean","Southern Ocean")){
  ...
  curr_dist <- vegdist(curr_mat,method="bray")
...
all.abund.plot1 <- ggplot(filter(plot.som_rpkm,SuperRegion%in%c("Estuarine","Coastal")),aes(x=Sample,y=rel_rpkm,fill=cluster))+
...
all.abund.plot2 <- ggplot(filter(plot.som_rpkm,SuperRegion%in%c("Oligotrophic Open Ocean")),aes(x=Sample,y=rel_rpkm,fill=cluster))+
  geom_point(aes(x=Sample,y=1.005,color=TARA),size=2,alpha=0.5)+
  scale_color_manual(values=c("black"),na.translate=FALSE)+
    geom_bar(position="stack",stat="identity",width=1)+
    coord_cartesian(ylim=c(0,1.01), expand=FALSE)+
...
all.abund.plot3 <- ggplot(filter(plot.som_rpkm,SuperRegion%in%c("Oligotrophic Seas","Southern Ocean")),aes(x=Sample,y=rel_rpkm,fill=cluster))+
  geom_point(aes(x=Sample,y=1.005,color=TARA),size=2,alpha=0.5)+
  scale_color_manual(values=c("black"),na.translate=FALSE)+
    geom_bar(position="stack",stat="identity",width=1)+
    coord_cartesian(ylim=c(0,1.01),expand=FALSE)+
    scale_fill_manual(values=cluster_colors, labels = c("1: slow - intermediate","2: intermediate","3: intermediate - fast","4: intermediate","5: slow","6: fast","7: intermediate","8: slow - intermediate"))+
    theme_bw()+
  labs(x="",fill="Cluster")+
...
#Version of above plots where we only look at estuarine + coastal + TARA stations
...
tara.som_rpkm <- sample.som_rpkm%>%
  mutate(TARA=case_when(Sample%in%tara.samples ~ "yes",
                                        .default=NA))%>%
  filter(SuperRegion %in% c("Estuarine","Coastal") || TARA == "yes")

#Load the NMDS clustering of the TARA samples to identify ecotype groups
load(file="Publication_Data/NMDS_clusters.RData")
ecotype_info <- som_temp%>%
  ungroup()%>%
  select(Sample, k3)

tara.som_rpkm <- left_join(tara.som_rpkm, ecotype_info,
                           by = "Sample")%>%
  distinct(Sample, cluster, .keep_all = TRUE)%>%
  mutate(NMDS_group = case_when(
    k3 == 1 ~ "Warm, oligotrophic",
    k3 == 2 ~ "Warm, elevated Chl",
    k3 == 3 ~ "Cold, high nutrients",
    .default = NA
  ))

tara_order <- c()
for(i in c("Estuarine","Coastal",1,2,3)){
  curr_df <- tara.som_rpkm%>%filter(SuperRegion==i || k3 == as.numeric(i))%>%select(cluster,Sample,rel_rpkm)%>%pivot_wider(names_from=cluster,values_from=rel_rpkm)
  curr_stations <- curr_df$Sample%>%as.vector()
  curr_mat <- curr_df[,-1]%>%as.matrix()
  curr_dist <- vegdist(curr_mat,method="bray")
  curr_clust <- hclust(curr_dist,method="mcquitty")
  ...
  curr_order <- curr_clust$order
  tara_order <- c(tara_order,curr_stations[curr_order])
}

#Now rearrange according to this order and plot
tara.som_rpkm$Sample <- factor(tara.som_rpkm$Sample,levels=all_order)


#Plot the three regions with lots of data as one group followed by the two small ones as another and stitch together with patchwerk
tara.abund.plot1 <- ggplot(filter(tara.som_rpkm,SuperRegion%in%c("Estuarine","Coastal","Oligotrophic Open Ocean")),aes(x=Sample,y=rel_rpkm,fill=cluster))+
  scale_color_manual(values=c("black"),na.translate=FALSE)+
    geom_bar(position="stack",stat="identity",width=1)+
...
tara.abund.plot2 <- ggplot(filter(tara.som_rpkm,!is.na(k3)),aes(x=Sample,y=rel_rpkm,fill=cluster))+
  geom_bar(position="stack",stat="identity",width=1)+
    coord_cartesian(ylim=c(0,1),expand=FALSE)+
    scale_fill_manual(values=cluster_colors, labels = c("1: slow - intermediate","2: intermediate","3: intermediate - fast","4: intermediate","5: slow","6: fast","7: intermediate","8: slow - intermediate"))+
    theme_bw()+
  labs(x="",fill="Cluster")+
    facet_grid(cols=vars(NMDS_group),scales="free_x")+
    theme(axis.text.x=element_blank(),axis.ticks.x=element_blank(),panel.grid=element_blank(),axis.text.y=element_blank(),axis.ticks.y=element_blank(),axis.title.y=element_blank())+
  guides(color="none", fill=guide_legend())+
  custom_theme

(tara.abund.plot1 | tara.abund.plot2) + plot_layout(widths=c(1,1),guides="collect")

#Version of the chart with just one bar for the mean abundance profile per region
tara.mean.rpkm <- tara.som_rpkm%>%
  mutate(SuperRegion = case_when(
    !is.na(NMDS_group) ~ NMDS_group,
    .default = SuperRegion
  ), 
  SuperRegion = factor(SuperRegion, levels = c("Estuarine","Coastal","Cold, high nutrients","Warm, elevated Chl","Warm, oligotrophic")))%>%
  group_by(SuperRegion, cluster)%>%
  summarise(rel_rpkm = mean(rel_rpkm))%>%
  filter(SuperRegion != "Oligotrophic Open Ocean")

tara.one.bar.plot <- ggplot(tara.mean.rpkm,aes(x=SuperRegion,y=rel_rpkm,fill=cluster))+
  geom_bar(position="stack",stat="identity",width=1)+
    coord_cartesian(ylim=c(0,1),expand=FALSE)+
    scale_fill_manual(values=cluster_colors, labels = c("1: slow - intermediate","2: intermediate","3: intermediate - fast","4: intermediate","5: slow","6: fast","7: intermediate","8: slow - intermediate"))+
    theme_bw()+
...
region.boot_df <- region.boot_df%>%mutate(growth=case_when(cluster==6 ~ "Fast", cluster%in%c(1,8) ~ "Slow - Intermediate",cluster==5 ~ "Slow", cluster %in% c(2,4,7) ~ "Intermediate", cluster == 3 ~ "Intermediate - Fast"))
region.boot_df$growth <- factor(region.boot_df$growth,levels=c("Slow","Slow - Intermediate","Intermediate","Intermediate - Fast","Fast"))

#Plot the bootstrapped output
regional.abund.plot <- ggplot(region.boot_df,aes(x=cluster,y=boot_mean,fill=growth))+
    geom_bar(stat="identity")+
    scale_fill_manual(values=c("#619CFF","lightblue","#00BA38","yellowgreen","#F8766D"))+
    geom_hline(yintercept=0,lty=2)+
  ...
  guides(fill = guide_legend(position="bottom", override.aes=list(c(nrow=1))))
...
boot_df <- boot_df%>%mutate(growth=case_when(cluster==6 ~ "Fast", cluster%in%c(1,8) ~ "Slow - Intermediate",cluster==5 ~ "Slow", cluster %in% c(2,4,7) ~ "Intermediate", cluster == 3 ~ "Intermediate - Fast"))
boot_df$growth <- factor(boot_df$growth,levels=c("Slow","Slow - Intermediate","Intermediate","Intermediate - Fast","Fast"))
...
###   ASV analysis
```{r Analyzing ASV data, echo=FALSE}
#Specific code for generating some plots for P16 transect -> generalize and functionalize parts that are not already functionalized at some point
#Read in all necessary data objects
P16.abund.data <- read_tsv(file="Data/221118-1030_P16N-S_3.84-fold-18S-correction_merged_16S_18S_proportions.QCd.prok-nonphoautototrophic.tsv")
P16.metadata <- read_csv(file="Data/2.20221118_P16NS_Sample_Metadata_Final.csv")
blast_data <- read_tsv(file="Data/carvewe-16S-grump-asv-blast.tsv")

#Filter out the high variance genomes
blast_data <- blast_data%>%
  filter(str_detect(sseqid, paste(genome_IDs, collapse = "|")))

#Filter the BLAST data to desired thresholds
blast_98_distinct <- blast_data%>%
  mutate(genome = str_split(sseqid, "-")%>%map_chr(1))%>%
  filter(pident >= 98.0, qcovhsp==100.0)%>%
  arrange(desc(pident))%>%
  distinct(qseqid, .keep_all = TRUE)

#Process abundance data, compute relative abundances
P16.processed <- process.GRUMP.data(P16.abund.data)

P16.computed <- compute.ASV.abundances(blast_98_distinct, cluster_df, P16.processed)
P16.analyzed <- analyze.rel.abundances(P16.computed[[1]], use.metadata = FALSE)
P16.rel.abund <- P16.analyzed[[2]]

#Calculate the per sample relative abundance total
per_samp_ASV <- colSums(select(P16.abund.data, where(is.double)))%>%
  data.frame(row.names = NULL, Stations = names(.))%>%
  rename(community_frac = ".")%>%
  select(Stations,everything())

#
P16.rel.abund <- left_join(P16.rel.abund, per_samp_ASV, by = "Stations")
P16.rel.abund <- left_join(P16.rel.abund, select(P16.metadata, c(Corrected_DNA_ID, Depth)), by = join_by(Stations==Corrected_DNA_ID))

#
P16.stations.by.NAfrac <- P16.rel.abund%>%
  group_by(Stations)%>%
  arrange(desc(NA_frac))%>%
  distinct(Stations, .keep_all=TRUE)

#Generate some interesting plots
station.info.plot <- ggplot(P16.stations.by.NAfrac, aes(x=Depth, y=NA_frac, color=community_frac))+
  geom_point(size=4)+
  geom_vline(xintercept = 150, size = 1, linetype = 2)+
  theme_bw()+
  custom_theme+
  scale_color_viridis_c(option="plasma")+
  labs(y = "Unassigned community fraction", color = "Fraction of\nTotal Community")

P16.abund.community.frac.plot <- ggplot(P16.rel.abund,aes(x=Stations,y=perc,fill=clusters))+
    geom_bar(position="stack",stat="identity",width=1)+
    geom_point(aes(x=Stations,y=1.005,color=community_frac),size=2,alpha=0.5)+
    scale_color_viridis_c(option="plasma")+
    coord_cartesian(ylim=c(0,1.01),expand=FALSE)+
    scale_fill_manual(values=c("#cc5a43","#7764cb","#75ab3d","#c062bb","#54a77a","#c75980","#b68f40","#678ecd"), na.value = "gray80")+
    theme_bw()+
    labs(x="",fill="Cluster", y = "Relative Abundance")+
...
order.P16.rel.abund <- P16.rel.abund%>%
  mutate(Stations = as.factor(Stations),
         Stations = fct_reorder(Stations, Depth))%>%
  mutate(Depth = case_when(
    Depth >= 500.0 ~ 500.0,
    .default = Depth
  ))
  
P16.abund.depth.plot <- ggplot(filter(order.P16.rel.abund, !is.na(Depth)),aes(x=Stations,y=perc,fill=clusters))+
    geom_bar(position="stack",stat="identity",width=1)+
    geom_point(aes(x=Stations,y=1.005,color=Depth),size=2)+
    scale_color_viridis_c(option="plasma", breaks=c(50, 150, 250, 500), labels = c("50", "150", "250", ">500"))+
    coord_cartesian(ylim=c(0,1.01),expand=FALSE)+
    scale_fill_manual(values=c("#cc5a43","#7764cb","#75ab3d","#c062bb","#54a77a","#c75980","#b68f40","#678ecd"), na.value = "gray80")+
    theme_bw()+
    labs(x="Station",fill="Cluster", y = "Relative Abundance")+
    theme(axis.text.x=element_text(angle=45,size=4.5,hjust=1,vjust=1.1),axis.ticks.x=element_blank(),panel.grid=element_blank())+
    guides(fill=guide_legend(override.aes = list(shape=NA)), color=guide_legend())+
    custom_theme
...
main.text.table=data.frame(cluster=c(1:num_clusters),num_genomes=cluster_df%>%group_by(clusters)%>%summarise(count=n())%>%select(count)%>%as.matrix()%>%as.vector())%>%mutate(designation=c("Slow-Intermediate","Intermediate","Intermediate-Fast","Intermediate","Slow","Fast","Intermediate","Slow-Intermediate"),limiting_substrates=c(
  ...
  c("Marinisomatales, Pelagibacterales"),
  c("Rhodospirillales, Burkholdeirales"),
  c("Sphingomonadales, Rhodospirillales"),
  c("Flavobacteriales, Cytophagales"),
  c("Opituales, Pelagibacterales"),
  c("Enterobacterales, Verrucomicrobiales"),
  c("Cytophagales, Pelagibacterales"),
  c("Pelagibacterales, Marinisomatales")
),top2_regions=c(
  c("Estuarine, Oligotrophic Seas"),
  c("Estuarine, Coastal"),
  c("Estuarine, Coastal"),
  c("Coastal, Oligotrophic Open Ocean"),
  c("Southern Ocean, Oligotrophic Seas"),
  c("Estuarine, Coastal"),
...
wilcox_df <- growth.wilcoxon.list[[2]]$p.value%>%
  melt(., na.rm = TRUE)%>%
  rename(`p adj` = value)%>%
  mutate(`Cluster Comparison` = paste0(Var1, "-", Var2))%>%
  select(`Cluster Comparison`, `p adj`)

p.value.table <- wilcox_df
rownames(p.value.table) <- c()
