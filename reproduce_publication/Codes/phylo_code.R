...
##' 
gheatmap2 <- function(p, data, offset=0, width=1, low="green", high="red", color="white",
  ...
  dd[[".panel"]] <- factor("Tree")
    p2 <- p + geom_tile(data=dd, aes(x, y, fill=value, color = value), width=width, inherit.aes=FALSE)
  if (is(dd$value,"numeric")) {
    p2 <- p2 + scale_fill_gradient(low=low, high=high, na.value=NA, name = legend_title) +
      scale_color_gradient(low=low, high=high, na.value=NA, name = legend_title) # "white")
  } else {
    p2 <- p2 + scale_fill_discrete(na.value=NA, name = legend_title) +
      scale_color_discrete(na.value=NA, name = legend_title) #"white")
  }
...
gheatmap_hidden <- function(p, data, offset=0, width=1, low="green", high="red", color="white",
                      colnames=TRUE, colnames_position="bottom", colnames_angle=0, colnames_level=NULL,
                      colnames_offset_x = 0, colnames_offset_y = 0, font.size=4, family="", hjust=0.5, legend_title = "value") {
  ...
  dd$width <- width
  dd[[".panel"]] <- factor("Tree")
  p2 <- p + geom_tile(data=dd, aes(x, y, fill=value, color = value), width=width, inherit.aes=FALSE, show.legend = FALSE)
  if (is(dd$value,"numeric")) {
    p2 <- p2 + scale_fill_gradient(low=low, high=high, na.value=NA, name = legend_title) +
      scale_color_gradient(low=low, high=high, na.value=NA, name = legend_title) # "white")
  } else {
